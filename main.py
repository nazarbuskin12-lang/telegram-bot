...
async def gnome_start(message: types.Message, state: FSMContext):
    ...
    if not user:
        ...
        if is_verified(user_id):
            ...
            await message.answer("⏳ Ваша верификация ещё не пройдена. Дождитесь подтверждения администратора.")
...
async def reg_bank(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "📸 **ВЕРИФИКАЦИЯ**\n\n"
        "Пожалуйста, отправьте фото паспорта (разворот с фото и данными).\n\n"
...
async def reg_verification(message: types.Message, state: FSMContext):
    ...
    # Начисление бонуса пригласившему (500₽)
...
async def gnome_work_bank(callback: types.CallbackQuery, state: FSMContext):
    ...
    if existing:
        await callback.message.edit_text("❌ У вас уже есть активная заявка на работу. Дождитесь её обработки.")
        await state.clear()
        await callback.answer()
        return
    
    cur.execute('''
        INSERT INTO work_requests (user_id, bank, status)
        VALUES (?, ?, 'pending')
    ''', (callback.from_user.id, bank))
    conn.commit()
    
    name = get_gnome_name(callback.from_user.id)
    await callback.message.edit_text(f"✅ Заявка на работу отправлена, {name}! Администратор рассмотрит её.")
    await state.clear()
    await callback.answer()
...
async def admin_password(message: types.Message, state: FSMContext):
    if message.text == ADMIN_PASSWORD:
        ...
        await message.answer("✅ Вход выполнен успешно!", reply_markup=admin_kb)
        await state.clear()
    else:
...
async def admin_requests(message: types.Message):
    if not is_admin_verified(message.from_user.id) and not is_master(message.from_user.id):
        await message.answer("⛔ У вас нет доступа. Пройдите авторизацию: /start")
        return
...
async def approve_request(callback: types.CallbackQuery):
    if not is_admin_verified(callback.from_user.id) and not is_master(callback.from_user.id):
    ...
    if row:
        ...
        await gnome_bot.send_message(row[0], f"✅ {name}, ваша заявка на работу одобрена!")
    await callback.answer()
...
async def reject_request(callback: types.CallbackQuery):
    ...
    if row:
        ...
        await gnome_bot.send_message(row[0], f"❌ {name}, ваша заявка на работу отклонена.")
    await callback.answer()

@admin_dp.message(F.text == "🆕 Верификация")
async def admin_verify_list(message: types.Message):
    if not is_admin_verified(message.from_user.id) and not is_master(message.from_user.id):
        await message.answer("⛔ У вас нет доступа. Пройдите авторизацию: /start")
        return
    
    cur.execute('SELECT id, telegram_id, full_name FROM gnomes WHERE verified = 0 AND status = "pending"')
    pending = cur.fetchall()
    if not pending:
        await message.answer("📭 Нет гномов на верификации")
        return
    
    for p in pending:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"verify_{p[1]}"),
             InlineKeyboardButton(text="❌ Отклонить", callback_data=f"reject_verify_{p[1]}")]
        ])
        await message.answer(f"🆕 Гном: {p[2]}\nID: {p[1]}", reply_markup=kb)
...
async def admin_verify_accept(callback: types.CallbackQuery):
    ...
    conn.commit()
    name = get_gnome_name(user_id)
    await callback.message.edit_text(f"✅ Гном {name} верифицирован")
    await gnome_bot.send_message(user_id, f"✅ {name}, ваша верификация пройдена! Теперь вы можете работать.")
    await callback.answer()
...
async def admin_gnomes(message: types.Message):
    ...
    cur.execute('SELECT id, full_name, phone, balance, verified FROM gnomes WHERE status = "active"')
    gnomes = cur.fetchall()
    if not gnomes:
    ...
    for g in gnomes:
        status = "✅" if g[4] else "⏳"
        text += f"{status} {g[1]} | {g[2]} | баланс: {g[3]} ₽\n"
...
async def admin_pay_process(message: types.Message, state: FSMContext):
    try:
        ...
        row = cur.fetchone()
        if not row:
            await message.answer("❌ Гном с таким ID не найден")
            return
...
async def admin_stats(message: types.Message):
    ...
    pending_work = cur.fetchone()[0]
    
    await message.answer(
        f"📊 **Статистика**\n\n"
        f"├ Гномов: {total_users}\n"
        f"├ Общий баланс: {total_balance} ₽\n"
        f"└ Заявок на работу: {pending_work}"
    )
