...
class AdminStates(StatesGroup):
    ...
    waiting_withdraw_decision = State()
...
async def gnome_start(message: types.Message, state: FSMContext):
    ...
    if not user:
        ...
        await message.answer(f"⏳ Привет, {user[2]}! Верификация не пройдена.")

@gnome_dp.message(GnomeReg.full_name)
async def reg_full_name(message: types.Message, state: FSMContext):
    await state.update_data(full_name=message.text)
    await message.answer("📞 Введите номер телефона (+79991234567):")
...
async def reg_phone(message: types.Message, state: FSMContext):
    ...
    if not phone.startswith('+') or not phone[1:].isdigit():
        await message.answer("❌ Неверный формат. Введите +79991234567")
        return
    await state.update_data(phone=phone)
    code = generate_sms_code(phone)
    await message.answer(f"📱 Код подтверждения: {code}\nВведите его для верификации:")
...
async def gnome_balance(message: types.Message):
    if not is_verified(message.from_user.id):
        return await message.answer("❌ Вы не прошли верификацию.")
    await message.answer(f"💰 Баланс: {get_gnome_balance(message.from_user.id)} ₽")
...
async def gnome_work_bank(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📞 Введите номер телефона, к которому привязан банк:")
...
async def gnome_work_phone(message: types.Message, state: FSMContext):
    ...
    conn.commit()
    await message.answer("✅ Заявка на работу отправлена администратору!")
    await state.clear()
...
async def withdraw_amount(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("📞 Введите номер телефона для вывода:")
...
async def withdraw_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer("🏦 Введите банк получателя:")
...
async def withdraw_bank(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Заявка на вывод {data['amount']} ₽ отправлена! Средства заблокированы.")
    await state.clear()

@gnome_dp.message(F.text == "✅ Пришли деньги")
async def gnome_money_received(message: types.Message, state: FSMContext):
    await message.answer("💰 Введите сумму, которая пришла на карту:")
...
async def deal_amount(message: types.Message, state: FSMContext):
    try:
        amount = int(message.text)
        await state.update_data(amount=amount)
        await message.answer("💳 Введите текущий баланс карты:")
...
async def deal_balance(message: types.Message, state: FSMContext):
    ...
    if deal:
        ...
        conn.commit()
    await message.answer("✅ Информация отправлена администратору!")
...
async def gnome_money_not_received(message: types.Message):
    await message.answer("⏳ Ожидайте, администратор проверит статус платежа.")
...
async def admin_requests(message: types.Message):
    ...
    for r in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Принять", callback_data=f"work_accept_{r[0]}"),
...
async def admin_send_deal(message: types.Message, state: FSMContext):
    await message.answer("📝 Введите текст сделки в формате:\n🔥Создана новая сделка\no3q92iebv952u643\nДата: 19 апреля 2026 г. 23:15\n+79130264139(Голованов Денис)-Тинькофф\nСумма 10395,00 ₽")
    await state.set_state(AdminStates.waiting_amount)

@admin_dp.message(AdminStates.waiting_amount)
async def admin_deal_text(message: types.Message, state: FSMContext):
    cur.execute('SELECT id FROM gnomes WHERE status = "active"')
    gnomes = cur.fetchall()
    if not gnomes:
        await message.answer("❌ Нет активных гномов")
        return
    for g in gnomes:
        cur.execute('INSERT INTO deals (gnome_id, deal_text, status) VALUES (?, ?, "waiting_payment")', (g[0], message.text))
        conn.commit()
        await gnome_bot.send_message(g[0], f"🆕 **Новая сделка!**\n\n{message.text}\n\nПосле поступления денег нажмите «Пришли деньги»", reply_markup=gnome_deal_kb)
    await message.answer(f"✅ Сделка отправлена {len(gnomes)} гномам")
    await state.clear()
...
async def admin_gnomes(message: types.Message):
    if not is_admin_verified(message.from_user.id):
        return
    cur.execute('SELECT full_name, phone, balance FROM gnomes WHERE status = "active"')
    rows = cur.fetchall()
    if not rows:
